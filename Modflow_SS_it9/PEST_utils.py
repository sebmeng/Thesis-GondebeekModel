...
def gen_ppoints(model_ws,plist, spacing, ix, iy, lx, ly, gsf_file, gsf_dim, nrow=None, ncol=None,trans_f_dict=None):
...
def setup_plproc_auto2D(model_ws,gsf_filename,sim,plist,unit_dict,zone_array,every_n_cell=None,truncate_values=False):
    ...
    with open(new_pst,'w') as f:
        ...
        for i in range(icount + 1, len(lines)):
            if i < icount + 1 + npar:
                ...
                f.write("log"+" "*(13-len("log")))
                f.write("factor"+" "*(13-len("factor")))
...
def pstobswt(model_ws,or_pst,obsgroup_name, factor, new_pst):
    ...
    with open(new_pst,'w') as f:
        ...
        for i in range(iline + 1, iline + 1 + int(lines[3].split()[1])):
            ...
            for item in range(2):
                f.write(obs_items[item] + " " * (14 - len(obs_items[item])))
            if obsgroup == obsgroup_name:
                f.write("0"+" "*13)
            else:
                new_weight = float(factor)*float(obs_items[2])
                f.write(str(new_weight) + " " * (14 - len(str(new_weight))))
            f.write(obs_items[3])
            f.write('\n')
...
def mfmod2obs_in(model_ws,gsf_file, coord, welllist, headobsfile,hdsbinaryfile,thresh, initialdate, nlay,extrap, headsimfile,precision=None):
    ...
    if precision == "d":
        os.system("mod2obs2.exe < "+instruction_file)
    else:
        os.system("mod2obs.exe < "+instruction_file)
...
def pstregwt(model_ws, or_pst,reggroups, new_pst):
    """
    Assigns weights of zero to the selected reggroups
    :param or_pst: original pst
...
def start_workers(pstfile,worker_dir,num_workers=None,worker_root="..",cleanup=True):
    """
    ...
    args = ['PEST_HP', pstfile, "/h", ":{0}".format(port)]
    ...
    for i in range(num_workers):
        ...
        args = ['PEST_HP', pstfile, "/h", tcp_arg]
        cwd = new_worker_dir
...
def well_bc_tpl(dir,sim,disc_neg=False):
    """
            ...
            if disc_neg:
                if iflux[iact]>0:
                    ...
                    pwf.write(" {0:s} = {1:f} * {2:s}\n".format(parname_tx
                                                        ,iflux[iact],
                                                        str(iper+1)+"_wm"+str(int(iiwels[iact]))))
                    ipar=ipar+1
                else:
